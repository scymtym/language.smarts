...
(defclass sm***s-document (lsp::document)
...
(defmethod (setf lsp::%text) :after ((text string) (document smarts-document))
  (handler-case
...
(defun language-server (&rest args &aux (input *standard-input*) (output *standard-output*))
  ...
  (let* ((connection (protocol.language-server.connection:make-connection
                      input output))
         (context    (make-instance 'lsp::context :connection      connection
                                                  :workspace-class 'smarts-workspace)))
    (trace "PROTOCOL.LANGUAGE-SERVER.PROTOCOL")
    (lsp::process-requests connection context)))
